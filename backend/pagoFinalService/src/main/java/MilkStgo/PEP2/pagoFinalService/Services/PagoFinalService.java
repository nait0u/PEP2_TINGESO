package MilkStgo.PEP2.pagoFinalService.Services;

import MilkStgo.PEP2.pagoFinalService.Entities.PagoFinalEntity;
import MilkStgo.PEP2.pagoFinalService.Models.ProveedorModel;
import MilkStgo.PEP2.pagoFinalService.Respositories.PagoFinalRepository;
...
public class PagoFinalService {
    ...
    @Autowired
    PagoFinalRepository pagoFinalRepository;
    ...
    public void guardarPlanilla(String fecha, String codigo, String nombre, String kg_leche,
                                ...
                                String monto_final)
    {
        PagoFinalEntity pagoFinal = new PagoFinalEntity();
        pagoFinal.setFecha(fecha);
        pagoFinal.setCodigo(codigo);
        pagoFinal.setNombre(nombre);
        pagoFinal.setKg_leche(kg_leche);
        pagoFinal.setDias(dias);
        pagoFinal.setPromedio_kg(promedio_kg);
        pagoFinal.setVariacion_leche(variacion_leche);
        pagoFinal.setGrasa(grasa);
        pagoFinal.setVariacion_grasa(variacion_grasa);
        pagoFinal.setSolidos(solidos);
        pagoFinal.setVariacion_st(variacion_st);
        pagoFinal.setPago_leche(pago_leche);
        pagoFinal.setPago_grasa(pago_grasa);
        pagoFinal.setPago_solidos(pago_solidos);
        pagoFinal.setBonif_freq(bonif_freq);
        pagoFinal.setDct_variacion_leche(dct_variacion_leche);
        pagoFinal.setDct_variacion_grasa(dct_variacion_grasa);
        pagoFinal.setDct_variacion_st(dct_Variacion_st);
        pagoFinal.setPago_total(pago_total);
        pagoFinal.setMonto_retencion(monto_retencion);
        pagoFinal.setMonto_final(monto_final);
        pagoFinalRepository.save(pagoFinal);
    }

    public PagoFinalEntity getAll()
    {
        return pagoFinalRepository.find().get(0);
    }

    @Generated
    public void eliminarData(ArrayList<PagoFinalEntity> datas)
    {
        pagoFinalRepository.deleteAll(datas);
    }
    ...
    public PagoFinalEntity generarPlanilla(String codigo) {
        ...
        PagoFinalEntity pagoFinal = getAll();
